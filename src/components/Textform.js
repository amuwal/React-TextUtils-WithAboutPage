...
export default function Textform(props) {
  ...
  return (
    <div className={`container  border ${props.mainClasses}`} id="main">
     ...
     <p id="summary" className={`${props.summaryClasses}`}>{text.split(" ").filter((element)=> {return element.length!==0}).length} words and {text.replace(/\s{2,}/g, ' ').trim().length} characters</p>
